...
```{r}
instacart_df |>
  plot_ly (x = ~product_name, y = ~order_hour_of_day, color = ~product_name, type = "box", colors = "viridis", text = ~paste("Product:", product_name,"Hour:", sprintf("%02d:00", order_hour_of_day), "<br>"),
           hoverinfo = "text",
           showlegend = FALSE) |>
...
instacart_df |>
  ...
  plot_ly( x = ~product_name, y = ~count, type ="bar", color = ~product_name, colors = "viridis", 
           text = ~paste("Product:", product_name, "<br>Count:", count),
           hoverinfo = "text",
           showlegend = FALSE) |>
...
plot_ly(data=heatmap_df, x= ~order_dow, y =~order_hour_of_day, z = ~count, type ="heatmap", colorscale = "viridis", hoverinfo = "text",
        text = ~paste(
          "Day:", order_dow, "<br>",
          "Hour:", sprintf("%02d:00", order_hour_of_day), "<br>",
          "Count:", count),
        hovertemplate = "%{text}<extra></extra>") |>
  layout(title = "Frozen Pizza Order Frequency by Day and Hour",
